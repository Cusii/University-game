...
def lectura():
    ...
    for linea in archivo_lectura:
        producto_lista = linea.strip().split(',')  # Eliminamos espacios en blanco y dividimos la línea en partes usando ',' como delimitador
        producto_final = [producto_lista[0], int(producto_lista[1]), int(producto_lista[2])]  # Creamos una lista con el nombre del producto y sus dos valores numéricos
...
def lectura_predeterminada():
    archivo_lectura=open("nuevosProductos.txt","r")
    lista_productos=[]

    for linea in archivo_lectura:
        producto_lista=linea.strip().split(',')
        producto_final=[producto_lista[0], int(producto_lista[1]), int(producto_lista[2])]
        lista_productos.append(producto_final)
...
def buscar_producto_aleatorio(lista_productos):
    ...
    for indice in range(len(producto_original)):
        if indice == 1:# Si estamos en el segundo indice (posición 1), añadimos una etiqueta aleatoria
            calidad = random.choice(["(premium)", "(economico)"])
            nuevo_producto.append(calidad)
        else: # Si no estamos en el segundo indice, simplemente copiamos el elemento original
...
def dameProducto(lista_productos, margen):
    while True:
        producto = buscar_producto_aleatorio(lista_productos)  # Utilizamos la función buscar_producto para obtener un producto aleatorio
...
def aparece(listaProductos,elemento):
...
def dameProductosAleatorios(producto, lista_productos, margen):
    ...
    while len(productos_seleccionados) <= 6: # Mientras no hayamos alcanzado la cantidad deseada de productos
        producto_random = buscar_producto_aleatorio(lista_productos)# Obtener un nuevo producto aleatorio
        ## controlar si no hay repetidos 
        if not aparece(productos_seleccionados,producto_random):#si no hay repetidos agrego el producto a la lista
            ...
            else: # Si ya garantizamos 2 productos, simplemente agregamos el nuevo producto
                productos_seleccionados.append(producto_random)
